...
import { FaSearch, FaTrash, FaEye, FaFilter, FaPrint, FaDownload, FaUserPlus, FaEdit, FaCheck, FaTimes } from "react-icons/fa";
...
const CustomerList = () => {
  ...
  // Mảng khách hàng mẫu với nhiều dữ liệu hơn
  ...
  const filteredCustomers = customers.filter(
    (c) => {
      const matchesSearch = c.id.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "Đã hoàn thành": return "text-green-400 bg-green-900/30";
      case "Đang điều trị": return "text-blue-400 bg-blue-900/30";
      case "Đã đặt lịch": return "text-yellow-400 bg-yellow-900/30";
      case "Chưa xác nhận": return "text-red-400 bg-red-900/30";
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex space-x-2">
            <button className="bg-indigo-600 text-white px-4 py-2 rounded flex items-center space-x-2 hover:bg-indigo-700 transition">
              <FaUserPlus />
              <span>Thêm khách hàng</span>
            </button>
            <button className="bg-gray-700 text-white px-3 py-2 rounded hover:bg-gray-600 transition">
              <FaPrint />
            </button>
            <button className="bg-gray-700 text-white px-3 py-2 rounded hover:bg-gray-600 transition">
              <FaDownload />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-r from-yellow-800 to-amber-800 p-4 rounded-lg shadow-lg">
            ...
            <p className="text-white text-2xl font-bold">
              {customers.filter(c => c.status === "Chưa xác nhận" || c.status === "Đã đặt lịch").length}
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-4 rounded-lg shadow-md mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="text-gray-300">
              <span>Hiển thị {currentRecords.length} / {filteredCustomers.length} khách hàng</span>
            </div>
          </div>
        </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="text-gray-200">
                {currentRecords.map((cust, index) => (
                  <tr key={cust.id} className={`border-t border-gray-700 hover:bg-gray-700/50 transition ${index % 2 === 0 ? 'bg-gray-800/50' : 'bg-gray-800'}`}>
                    ...
                    <td className="p-3">
                      <div className="font-medium">{cust.dateTime.split(' ')[0]}</div>
                      <div className="text-xs text-gray-400">{cust.dateTime.split(' ')[1]}</div>
                    </td>
                    ...
                    <td className="p-3">
                      {editingId === cust.id ? (
                        ...
                        <div className="flex items-center space-x-2">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(cust.status)}`}>
                            {cust.status}
                          </span>
                          <button 
                ...
                {currentRecords.length === 0 && (
                  <tr>
                    <td colSpan="8" className="p-4 text-center text-red-400 font-semibold">
              ...
              <div className="flex space-x-2">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                  <button
                    ...
                    className={`px-3 py-1 rounded ${currentPage === page ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-white hover:bg-gray-600'}`}
                  >
                    {page}
                  </button>
                ))}
